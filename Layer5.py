...
class Layer5_Spectrum(QWidget):
    def __init__(self):
        ...
        self.Layer5_Spectrum_layout.addWidget(self.add_column_button, 0, 2)
        self.Layer5_Spectrum_layout.addWidget(self.create_data_button, 1, 0)
        self.Layer5_Spectrum_layout.addWidget(self.select_db_table, 1, 1)
        self.Layer5_Spectrum_layout.addWidget(self.table_delete_button, 1, 2)
        self.Layer5_Spectrum_layout .addWidget(self.table, 2, 0, 1, 3)
    ...
    def LoadDataBase(self):
        ...
        # 確保表格存在
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='Layer5';")
        if cursor.fetchone() is None:
            connection.close()
            return  # 如果表格不存在，直接返回

        # 獲取表格的標題
        cursor.execute("PRAGMA table_info(Layer5);")
        ...
        # 獲取表格數據
        result = connection.execute("SELECT * FROM Layer5")
