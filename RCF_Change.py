...
class RCF_Change_Spectrum(QWidget):
    ...
    def __init__(self):
        ...
        self.RCF_Change_Spectrum_layout.addWidget(self.Form_clear_button, 0, 2)
        self.RCF_Change_Spectrum_layout.addWidget(self.create_data_button, 1, 0)
        self.RCF_Change_Spectrum_layout.addWidget(self.table_delete_button, 1, 1)
        self.RCF_Change_Spectrum_layout.addWidget(self.select_db_table, 1, 2)
        self.RCF_Change_Spectrum_layout.addWidget(self.add_column_button, 2, 0)
        self.RCF_Change_Spectrum_layout.addWidget(self.delete_column_button, 2, 1)
        self.RCF_Change_Spectrum_layout.addWidget(self.delete_select_column_button, 2, 2)
        self.RCF_Change_Spectrum_layout.addWidget(self.save_table_button, 3, 0)
        self.RCF_Change_Spectrum_layout.addWidget(self.insert_column_button, 3, 1)
        self.RCF_Change_Spectrum_layout.addWidget(self.paste_button, 3, 2)
        self.RCF_Change_Spectrum_layout.addWidget(self.table, 4, 0, 1, 3)
    ...
    def LoadDataBase(self):
        ...
        # 確保表格存在
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='RCF_Change_範例檔勿刪';")
        if cursor.fetchone() is None:
            connection.close()
            return  # 如果表格不存在，直接返回

        # 獲取表格的標題
        cursor.execute("PRAGMA table_info('RCF_Change_範例檔勿刪');")
        ...
        # 獲取表格數據
        result = connection.execute("SELECT * FROM 'RCF_Change_範例檔勿刪'")
