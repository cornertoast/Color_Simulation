...
class RCF_Fix_Spectrum(QWidget):
    ...
    def LoadDataBase(self):
        ...
        # 確保表格存在
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='RCF_Fix';")
        if cursor.fetchone() is None:
            connection.close()
            return  # 如果表格不存在，直接返回

        # 獲取表格的標題
        cursor.execute("PRAGMA table_info(RCF_Fix);")
        ...
        # 獲取表格數據
        result = connection.execute("SELECT * FROM RCF_Fix")
